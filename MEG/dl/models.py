...
class Sample(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(Flatten_MEG(), nn.Linear(204 * 1001, 1), nn.ReLU(), nn.Dropout(1))
    ...
    def __init__(self, n_layer, kernel_size, activation, bias=False):
        ...
        self.block = nn.Sequential(
            *[
                ...
                for layer in [
                    nn.Conv2d(self.in_channel[i], self.out_channel[i], kernel_size=[kernel_size[i], 1], bias=False),
...
class TemporalBlock(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, kernel_size, max_pool=None, activation="relu"):
        ...
        layers = [
            nn.Conv2d(self.input_channel, self.output_channel, kernel_size=[1, self.kernel_size], bias=False),
            Activation(self.activation),
...
class Temporal(nn.Module):
    ...
    def __init__(self, n_block, kernel_size, n_times, activation, max_pool=None, bias=False):
        ...
        self.temporal = nn.Sequential(
            *[
                TemporalBlock(
                    self.in_channel[i], self.out_channel[i], self.kernel_size[i], self.max_pool, self.activation
                )
                for i in range(n_block)
            ]
        )
...
class SCNN(nn.Module):
    ...
    def __init__(
        ...
        self.in_channel = (
            temporal_n_block * 16 * n_spatial_layer * 16 * self.temporal.n_times_
...
class RPS_SCNN(nn.Module):
    ...
    def __init__(
        ...
        self.in_channel = (
            temporal_n_block * 16 * n_spatial_layer * 16 * self.temporal.n_times_ + 204 * 6
        )  # TODO substitue the number of channel
        self.ff = MLP(self.in_channel, mlp_hidden, mlp_n_layer, mlp_dropout, activation)
...
class RPS_MLP(nn.Module):
    def __init__(self, in_channel=204, n_bands=6):
        ...
        self.ff = nn.Sequential(
            nn.Linear(in_channel * n_bands, 1024), nn.ReLU(), nn.Linear(1024, 1024), nn.ReLU(), nn.Linear(1024, 1)
        )
