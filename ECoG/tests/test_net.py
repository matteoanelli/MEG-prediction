...
def test_create_raw():
    ...
    assert raw.get_data().shape == np.shape(
        np.zeros((101, 10000))
    ), "Wrong shape, expected {}. Got instead {}".format(
        np.shape(np.zeros((101, 10000))), raw.get_data().shape
    )
...
def test_ECoG_dataset_shape():
    ...
    dataset = ECoG_Dataset(
        data_dir, file_name, finger=0, duration=1.0, overlap=0.8, rps=True
    )
    ...
    assert sample_data.shape == torch.Size(
        ...
        torch.Size([50, 1, 62, 501]), sample_data.shape
    )

    assert sample_target.shape == torch.Size(
        [50]
    ), "wrong target shape, data shape expected = {}, got {}".format(
        torch.Size([50]), sample_target.shape
    )

    assert sample_bp.shape == torch.Size(
        [50, 62, 6]
    ), "wrong target shape, data shape expected = {}, got {}".format(
        torch.Size([50, 62, 6]), sample_target.shape
    )

    # test without rps
    dataset = ECoG_Dataset(
        data_dir, file_name, finger=0, duration=1.0, overlap=0.8, rps=False
    )
...
def test_sampler():
    ...
    trainloader = DataLoader(
        train_set, batch_size=70, shuffle=True, num_workers=1
    )
    testloader = DataLoader(
        test_set, batch_size=15, shuffle=False, num_workers=1
    )
...
def test_Subset():
    ...
    validloader = DataLoader(
        valid_dataset, batch_size=10, shuffle=True, num_workers=1
    )
    testloader = DataLoader(
